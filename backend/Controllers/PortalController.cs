...
/// <summary>
/// Anonymous endpoints reached via personalized secure links in emails — for registered users
/// AND external contacts. Every access is validated (expiry, revocation) and audit-logged.
/// </summary>
[ApiController]
[Route("api/portal")]
public class PortalController : ControllerBase
{
    ...
    public async Task<IActionResult> Resolve(string token)
    {
        ...
        await _audit.LogAsync("link.accessed", t.Resource.ToString(), t.ResourceId,
            new { contactId = t.ContactId }, t.UserId,
        ...
        switch (t.Resource)
        {
            ...
            case LinkResource.Paper:
            {
                ...
                await _audit.LogAsync("paper.downloaded", "BoardPaper", p.Id,
                    new { version = v.VersionNumber, v.OriginalFileName, contactId = t.ContactId, via = "portal" }, t.UserId,
                    HttpContext.Connection.RemoteIpAddress?.ToString());
