  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
        <span className={`docket ${m.minutesStatus === 'Finalized' ? 'docket-final' : ''}`}>{m.meetingCode}</span>
        <h1 style={{ margin: 0 }}>{m.title}</h1>
      </div>
      <p style={{ color: 'var(--ink-soft)' }}>
        {m.type} meeting · {m.mode} · {new Date(m.scheduledAtUtc).toLocaleString()}{m.mode !== 'Online' && m.location ? ` · ${m.location}` : ''}{m.mode !== 'Physical' && m.videoLink ? <> · <a href={m.videoLink}>Join online</a></> : null}
      </p>
